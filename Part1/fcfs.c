...
int main(int argc, char *argv[]){
  ...
  while(1){
    if(temp == NULL){//we need a new process
      if(arrivalQ->head != NULL){
        if(t >= arrivalQ->head->arrivalTime){
          temp = q_dequeue(arrivalQ);
          //printf("Job %s has arrived. Running Time:%d.\n",readyQ->tail->name ,readyQ->tail->runTime);
        }
      }
    }
    //Currently running process get's attention.
    if(temp != NULL){
      (temp->cyclesExecuted)++; //"run" process 1 time slice
      if(temp->runTime == temp->cyclesExecuted){/*processes completed*/
    ...
    if(arrivalQ->head == NULL || t == simTime) break;
    t++;
    //printf("time is %d\n", t);
  }
